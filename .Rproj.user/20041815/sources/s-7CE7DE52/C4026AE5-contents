...
double g_function_weighted(arma::mat X_local, arma::vec Q_local, arma::vec weights_local, arma::vec u_local){
  ...
  arma::vec temp(n);
  double g = 0, sum_weights_local = 0, dot_u_local_Q_local = 0;
  for (i = 0; i < n; ++i){
    temp[i] = 0;
    for (j = 0; j < d; ++j){
      temp[i] = temp[i] + pow((Q_local[j] - X_local(i, j)), 2);
    }
    temp[i] = weights_local[i] * sqrt(temp[i]);
    
    g = g + temp[i];
    
    sum_weights_local = sum_weights_local + weights_local[i];
...
arma::vec spquantile(arma::mat Data, arma::vec Weights, int u_index, double c, arma::vec t_vector){
  ...
  for (j = 0; j < p; ++j){
    Quantile[j] = 0;
    for (k = 0; k < d_n; ++k){
      Quantile[j] = Quantile[j] + (Quantile_coefficients[k] * Eigenvectors_sorted_truncated(j, k));
    }
    Quantile[j] = Quantile[j] + Weighted_Mean[j];
  }
